// EquityPDFGenerator.js
import jsPDF from 'jspdf';

export const EquityPDFGenerator = {
    generateFilledPDF: (formData) => {
        ...
        doc.text('PACAM EQUITY FUND', pageWidth / 2, yPosition, { align: 'center' });
        yPosition += 8;
        ...
        EquityPDFGenerator.addFormRow(doc, 'FULL NAME (as printed on Fund Statement):', formData.fullName || '', leftMargin, yPosition, contentWidth);
        yPosition += 12;

        checkNewPage(15);

        // Client ID and Phone (side by side)
        EquityPDFGenerator.addFormRowSplit(doc,
        ...
        // Email
        EquityPDFGenerator.addFormRow(doc, 'EMAIL:', formData.email || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;

        checkNewPage(30);

        // Redemption Details Section
        EquityPDFGenerator.addSectionHeader(doc, 'REDEMPTION DETAILS (TO BE COMPLETED BY ALL CLIENTS)', leftMargin, yPosition);
        yPosition += 12;

        EquityPDFGenerator.addFormRow(doc, 'Value/Number of Units to be redeemed (in figures):', formData.unitsToRedeemFigures || '', leftMargin, yPosition, contentWidth);
        yPosition += 12;

        checkNewPage(15);

        EquityPDFGenerator.addFormRow(doc, 'Value/Number of Units to be redeemed (in words):', formData.unitsToRedeemWords || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;

        checkNewPage(30);

        // Payment Details Section
        EquityPDFGenerator.addSectionHeader(doc, 'PAYMENT DETAILS', leftMargin, yPosition);
        yPosition += 12;
        ...
        // Account Name
        EquityPDFGenerator.addFormRow(doc, 'ACCOUNT NAME:', formData.accountName || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;
        ...
        EquityPDFGenerator.addSectionHeader(doc, 'FOR FUND MANAGER\'S USE ONLY', leftMargin, yPosition);
        EquityPDFGenerator.addSectionHeader(doc, 'FOR REGISTRAR\'S USE ONLY', leftMargin + halfWidth + 10, yPosition);
        yPosition += 12;
        ...
        fundManagerFields.forEach((field, index) => {
            checkNewPage(12);
            EquityPDFGenerator.addFormRowSmall(doc, field, '', leftMargin, yPosition, halfWidth);
            yPosition += 10;
        });
        ...
        EquityPDFGenerator.addFormRow(doc, 'Processed by:', '', leftMargin, yPosition, contentWidth);
        yPosition += 15;

        checkNewPage(50);

        // Company Seal Section
        EquityPDFGenerator.addSectionHeader(doc, 'AFFIX COMPANY SEAL', leftMargin, yPosition);
        yPosition += 10;
        ...
        EquityPDFGenerator.addSectionHeader(doc, 'DETAILS OF ATTACHED UNIT CERTIFICATE', leftMargin, yPosition);
        yPosition += 12;
        ...
        EquityPDFGenerator.addFormRow(doc, 'Current Redemption:', formData.currentRedemption || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;

        checkNewPage(40);

        // Important Notes
        EquityPDFGenerator.addSectionHeader(doc, 'Note:', leftMargin, yPosition);
        yPosition += 10;
        ...
        doc.text(`Reference: PACAM-EQUITY-${Date.now()}`, pageWidth - rightMargin, yPosition, { align: 'right' });
