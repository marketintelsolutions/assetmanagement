// FixedIncomePDFGenerator.js
import jsPDF from 'jspdf';

export const FixedIncomePDFGenerator = {
    generateFilledPDF: (formData) => {
        ...
        doc.text('PACAM FIXED INCOME FUND', pageWidth / 2, yPosition, { align: 'center' });
        yPosition += 8;
        ...
        FixedIncomePDFGenerator.addFormRow(doc, 'FULL NAME (as printed on Fund Statement):', formData.fullName || '', leftMargin, yPosition, contentWidth);
        yPosition += 12;

        checkNewPage(15);

        FixedIncomePDFGenerator.addFormRowSplit(doc,
        ...
        FixedIncomePDFGenerator.addFormRow(doc, 'EMAIL:', formData.email || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;

        checkNewPage(30);

        // Redemption Details Section
        FixedIncomePDFGenerator.addSectionHeader(doc, 'REDEMPTION DETAILS (TO BE COMPLETED BY ALL CLIENTS)', leftMargin, yPosition);
        yPosition += 12;

        FixedIncomePDFGenerator.addFormRow(doc, 'Value/Number of Units to be redeemed (in figures):', formData.unitsToRedeemFigures || '', leftMargin, yPosition, contentWidth);
        yPosition += 12;

        checkNewPage(15);

        FixedIncomePDFGenerator.addFormRow(doc, 'Value/Number of Units to be redeemed (in words):', formData.unitsToRedeemWords || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;

        checkNewPage(30);

        // Payment Details Section
        FixedIncomePDFGenerator.addSectionHeader(doc, 'PAYMENT DETAILS', leftMargin, yPosition);
        yPosition += 12;
        ...
        FixedIncomePDFGenerator.addFormRow(doc, 'ACCOUNT NAME:', formData.accountName || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;
        ...
        FixedIncomePDFGenerator.addSectionHeader(doc, 'FOR FUND MANAGER\'S USE ONLY', leftMargin, yPosition);
        FixedIncomePDFGenerator.addSectionHeader(doc, 'FOR REGISTRAR\'S USE ONLY', leftMargin + halfWidth + 10, yPosition);
        yPosition += 12;
        ...
        fundManagerFields.forEach((field, index) => {
            checkNewPage(12);
            FixedIncomePDFGenerator.addFormRowSmall(doc, field, '', leftMargin, yPosition, halfWidth);
            yPosition += 10;
        });
        ...
        FixedIncomePDFGenerator.addFormRow(doc, 'Processed by:', '', leftMargin, yPosition, contentWidth);
        yPosition += 15;

        checkNewPage(50);

        // Company Seal Section
        FixedIncomePDFGenerator.addSectionHeader(doc, 'AFFIX COMPANY SEAL', leftMargin, yPosition);
        yPosition += 10;
        ...
        FixedIncomePDFGenerator.addSectionHeader(doc, 'DETAILS OF ATTACHED UNIT CERTIFICATE', leftMargin, yPosition);
        yPosition += 12;
        ...
        FixedIncomePDFGenerator.addFormRow(doc, 'Current Redemption:', formData.currentRedemption || '', leftMargin, yPosition, contentWidth);
        yPosition += 20;

        checkNewPage(40);

        // Important Notes
        FixedIncomePDFGenerator.addSectionHeader(doc, 'Note:', leftMargin, yPosition);
        yPosition += 10;
        ...
        doc.text(`Reference: PACAM-FIXED-INCOME-${Date.now()}`, pageWidth - rightMargin, yPosition, { align: 'right' });
