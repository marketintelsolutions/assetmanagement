// useFixedIncomeFormLogic.js
import { useState } from 'react';
import { FixedIncomePDFGenerator } from './FixedIncomePDFGenerator';
import { apiService, apiUtils } from '../../../../services/apiClient';

export const useFixedIncomeFormLogic = (fundManagerEmail) => {
    ...
    const generatePDF = () => {
        try {
            const pdfDoc = FixedIncomePDFGenerator.generateFilledPDF(formData);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await apiUtils.retry(
                () => apiService.fixedIncomeFund.submit(formData, pdfContent, fundManagerEmail),
                3,
                2000
            );
            ...
            let errorMessage = 'Failed to submit Fixed Income fund redemption form.\n\n';
    ...
    const handlePreviewPDF = () => {
        try {
            const pdfDoc = FixedIncomePDFGenerator.generateFilledPDF(formData);
            pdfDoc.save(`PACAM_FixedIncome_Redemption_Preview_${Date.now()}.pdf`);
        } catch (error) {
